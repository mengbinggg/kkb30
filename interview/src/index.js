...
function binarySearch2(arr, target, i, j) {
    let len = arr.length;
    if (len == 0 || !target)
        return -1;
    i = i || 0;
...
function prevOrderTraversal(tree) {
    let res = [];
    if (!tree)
        return res;
...
function findContinuousMaxChar1(str) {
    let res = {
        char: '',
        length: 0
    };
...
function findContinuousMaxChar2(str) {
    let res = {
        char: '',
        length: 0
    };
